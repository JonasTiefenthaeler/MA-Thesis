...
\begin{table}
    ...
    \begin{tabular}{c c c c c c c c c}
        ...
        \midrule
        smooth  & $181.0$   & -        & $0.23$   & $1.14$   & $1.14$   & -                     & $1.00$           & - \\
...
The wall-normal Reynolds stresses $v'v'$ are increased over the entire domain, in comparison to the smooth cases.
This is the case for all three surfaces.
However, as in the streamwise direction, the maximum wall-normal Reynolds stresses are attained at higher $y^+$ values.
The same findings can be applied to the azimuthal Reynolds stresses $w'w'$.
...
Surface $S1$ exhibits a peak value of $1.31$, $S2$ $1.56$ and $S3$ $1.57$.
The data indicates that an increase in roughness results in a shift of the energy peak outwards, away from the wall.
The peak value for $S1$ is at $y^+ \approx 23.0$, for $S2$ at $y^+ \approx 19.7$ and for $S3$ at $y^+ \approx 19.0$.
...
Furthermore, $S1$ and $S3$ show almost the same velocity at the virtual origin, while the velocity of $S2$ is significantly lower.
...
The wall-normal Reynolds stresses $v'v'$ behave almost identically between the three surfaces, except in the near wall region.
In this region, larger roughness height results in larger  wall-normal fluctuations.
The same holds for the azimuthal Reynolds stresses $w'w'$.
...
The roughness function decreases with decreasing roughness height and effective slope, which is in line with the other results presented in this table.
\begin{figure}[h!]
    \centering
    \begin{subfigure}[t]{0.495\textwidth}
        \centering
...
The overall increase of the wall-normal $v'v'$ and azimuthal $w'w'$ fluctuations is also present.
...
The  wall-normal stresses $v'v'$ and azimuthal stresses $w'w'$ are plotted in \autoref{subfig: Reynolds comp S1 Reynolds stresses vv}.
...
The wall-normal fluctuations $v'v'$ and the azimuthal fluctuations $w'w'$ behave very similar.
...
In \autoref{subfig: Reynolds comp S2 Reynolds stresses uu} the streamwise Reynolds stresses and turbulent shear stresses are plotted.
...
The wall-normal and azimuthal fluctuations $v'v'$ and $w'w'$ in \autoref{subfig: Reynolds comp S2 Reynolds stresses vv} exhibit the same trends across the Reynolds numbers for $S2$ as for $S1$.
...
The velocity profiles in \autoref{subfig: Reynolds comp S3 velocity} show a comparable behaviour to that exhibited by surfaces $S1$ and $S2$.
...
However, the maximum non-dimensionalised velocity $U^+$ increases with rising Reynolds number, contrary to $S1$ and $S2$.
...
At $Re_b = 19000$, a strong influence of dispersive stress is apparent since the turbulent stress is significantly decreased from $r/R \approx 0.7$ until it reaches approximately the same peak value as $Re_b = 11700$.
...
Furthermore, the streamwise fluctuations increase at the virtual origin, as they do for $S1$ and $S2$.
The shear stresses $u'v'$ behave analogously to $S2$.
As do the wall-normal fluctuations $v'v'$ and azimuthal fluctuations $w'w'$.
...
The same trend for $S3$ emerge, as observed for $S2$ and $S3$.
...
\begin{figure}[h]
    ...
    \end{subfigure}
    \\[-10pt]
    \caption[General relations of $\Delta U^+$ and $\tau_p$ over $k^+$]{General relations related to roughness height. Point colour depicting the simulation case. (a) Roughness function $\Delta U^+$ over roughness height $k^+$ for all 9 simulations performed. The black dashed line depicts the Hama roughness function.~(b) Pressure drag $\tau_p$ normalised in wall units over roughness height $k^+$.}
    \label{fig: general correlations}
\end{figure}
The roughness f
All cases are well below the Hama roughness function, which implies that all cases are not in the fully-rough regime.
...
The influence on the roughness function $\Delta U^+$ is however smaller, namely between $14\%$ for $Re_b = 5300$, $18\%$ for $Re_b = 11700$ and $17\%$ for $Re_b = 19000$.
Measured in absolute values, the difference in roughness function grows monotonically with the Reynolds number.
...
A first-order polynomial is fitted for each surface and depicted as solid black lines.
A clear relation between the effective slope of the surface and the increase in pressure drag is observed.
Surface $S1$ and $S2$ show the same pressure drag increase with a slope of $0.11$, while $S3$ shows a slower increase with a slope of $0.08$.
...
The general assumption of the roughness sublayer extending up to $r = 3k$~\citep{chungPredictingDragRough2021}, is approximately confirmed by the existing dispersive stresses in some cases in this thesis.
All cases using $Re_b = 5300$ show the influence of the dispersive stresses until approximately $r/R \approx k$, while at $Re_b = 19000$ the influence of the dispersive stresses reaches until $r/R \approx 3k$ for all surfaces.
However, the present data suggests that other factors, like surface parameters play a role in this observation.
This can be assumed since at $Re_b = 11700$ the range of influence is dependent on the surface.
The dispersive stresses have a substantially higher range of impact at $S1$ until $r/R \approx 3k$, while for $S2$ and $S3$ they play only a role until $r/R \approx 1.5k$.
