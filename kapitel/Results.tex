...
This shortening for $S2$ compared to the similar roughness height surface $S3$ could be explained by the surface gemoetry of $S2$, which generated using more Fourier modes.
...
The mean velocity profiles, plotted in \autoref{subfig: 19000 validation velocity profile}, show substantial wiggles in the outer region, which indicates a strongly shifted mean velocity away from the pipe centre. 
This effect is less pronounced in the mean velocity profiles of surface $S1$, which is counterintuitive, since $S1$ exhibits the most roughness.
A possible explanation for this observation could be a dampening influence on this due to the increased drag in this case.
The mean velocities for $S1$ and $S2$ collapse onto another, which does not imply convergence for $S1$ and $S2$ in that regard.
However, it can be observed, that the velocities at the virtual origin move closer together across the three Reynolds numbers considered in this thesis.
NOCH KURZ AUF VELOCITY DEFECTS EINGEHEN
...
All three show the expected slope of $1$ throughout the domain, except in the the near wall region.
The turbulent stresses exhibit the expected slope of $1$ only in the outer region.
...
The dispersive stresses for $S1$ and $S3$ display the same trend, however with lower magnitude for $S3$, which is expected due to the lower roughness height.
The viscous stresses show only a small influence at this Reynolds number.
The stay near zero until $r/R \approx 0.1$ for all three surfaces.
They rise to values between $0.21$ for $S1$ and $0.34$ for $S3$ at the virtual origin.
...
The premultiplied streamwise energy spectra in \autoref{fig: energy spectra 19000} show also show similar trends as for $Re_b = 11700$ and $Re_b = 5300$.
