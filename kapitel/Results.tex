...
\autoref{fig: comp Reb 5300} presents first- and second-order statistics for all three surfaces. 
...
\begin{table}
    ...
    \begin{tabular}{c c c c c c c c c}
        ...
        $S1$    & $487.4$   & $111.1$   & $9.19$                & $6.94$                        & $4.60$                    & $2.35$                    & $0.66$            & $5.51$\\
...
Additionally, the dominant wavelength is longer for smaller roughness heights, since it is $\lambda_z^+ \approx 557$ for $S1$ and $\lambda_z^+ \approx 638$.
The dominant wavelength is $\lambda_z^+ \approx 543$ for $S2$.
This shortening for $S2$ compared to the similar roughness height surface $S3$ could be explained by the surface geometry of $S2$, which is generated using more Fourier modes.
Due to the smaller roughness elements, the turbulence is broken up in smaller  sections, hence shorter energy wavelengths.
Furthermore, the energy is more distributed over the wavelength range for $S2$ and $S3$ than for $S1$.
Especially regions at high wavelengths and high $y^+$ values are more pronounced.
Additionally, it can be seen, that the contour line at $0.75$ enclose a larger area in the plots for $S2$ and $S3$ than for $S1$, which also implies a higher energy content overall.
...
Near the wall $S1$ and $S2$ collapse onto another, while $S3$ exhibits reduced values.
...
All three surfaces show the expected slope of $1$ throughout the domain, except in the near wall region.
...
The findings for the Reynolds stresses, which are displayed in \autoref{subfig: Reb 19000 reynolds stresses} are in line with $Re_b = 11700$ and $Re_b = 5300$.
...
However, the behaviour dominant wavelengths are different for $Re_b = 19000$.
...
\end{figure}
The development of the viscous drag with respect to the roughness height for an arbitrary $\theta$ position is plotted in \autoref{fig: roughness height and viscous stress Reb 19000 piro}.
...
It is interesting to note, that the maximum non-dimensionalised velocity $U^+$ is approximately equal for all three Reynolds numbers. 
...
Furthermore, the peak gets less pronounced for increasing Reynolds numbers, since the stresses reach a value at $y^+ = 0$ that is only slightly lower than the peak value.
...
This further support the interpretation of a shift of energy away from shorter wavelengths towards large scale motions. 
...
The energy content near the wall seems to increase slightly for higher Reynolds numbers.
This indicates stronger flow intensities in the near wall region for higher Reynolds numbers.
...
Furthermore, the dominant wavelength is longer for increased Reynolds numbers.
...
The mean velocity profiles in \autoref{subfig: Reynolds comp S2 velocity} show the clear, but expected trend, of an increased downshift of the log-law for an increasing Reynolds number, implying an increased Roughness function.
...
Contrary to $S1$, the turbulent stress for $Re_b = 5300$ and $Re_b = 11700$ behave the same until $r/R \approx 0.4$.
...
However, the maximum non-dimensionalised velocity $U^+$ increases with rising Reynolds number, which suggests a connection to the lower roughness for $S3$ and this particular observation.
...
\begin{figure}
    ...
    \caption[$\Delta U^+$ over $k^+$ ar all simulations]{$\Delta U^+$ over $k^+$ for all 9 simulations performed. Point colour depicting simulation case. The black dashed line depicts the Hame roughness function.}
    \label{fig: Delta U+ over y+}
\end{figure}
The roughness function $\Delta U^+$ over $y^+$ is plotted in \autoref{fig: Delta U+ over y+}.
...
The data suggests, that with a decreasing roughness height $k^+$, the energy is distributed towards the wall, despite the overall lower energy content.
