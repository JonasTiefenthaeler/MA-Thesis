...
This means, that all scales of turbulent motion are resolved from the larges eddies, down to the viscous scale.
...
The employed \acrshort{sem} method is originally proposed by~\cite{pateraSpectralElementMethod1984} and can be seen as a combination of the \acrfull{fem} and spectral methods to achieve a significantly higher accuracy and faster convergence. 
This high-order approach achieves the flexibility in spatial discretisation of the \acrshort{fem} and combining it with the high accuracy of spectral methods.
The high accuracy of spectral methods is due to the fast spectral convergence.
This is achieved, since the  value of a derivative at a gridpoint is not only dependent by points in its vicinity, but on all points of the domain.
...
It's a dimensionless value which is defined as
\begin{equation}
    c = \frac{u\Delta t}{\Delta x}
\end{equation}
and based on the work by~\cite{courantBerPartiellenDifferenzengleiehungen}.
...
If the maximum \acrshort{cfl} number in the domain is above or below the specified average, this formula is rearranged, such that a new time step can be computed.
