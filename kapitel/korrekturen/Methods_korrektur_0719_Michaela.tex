...
The computation time can be estimated as  $\approx Re^3$ for isotropic turbulence and $\approx Re^\frac{37}{14}$ for wall-bounded flows \citep{choiGridpointRequirementsLarge2012}.\fxinfo{Das ist ein sehr gutes Argument!}
...
Especially under rough wall conditions, it can be very challenging or impossible to obtain data about the flow behaviour in the vicinity of roughness\fxchange{meinst Du:  in the vicinity of the  wall? oder ???} features ZITAT.
...
The axis scaling is set to log-linear, , thus ensuring the exponential convergence of spectral methods is represented as a straight line.
...
This places the grid points denser at the element boundaries and more sparsely in the elements centres.
...
When integrating the \acrshort{nse} in\fxchange{ich glaube es heißt: over oder with respect to time}  time, a feasible time step size must be determined, such that the scheme is stable and produces correct results.
By setting the \acrfull{cfl} number, the necessary time step can be determined.
the  \acrfull{cfl} value is  dimensionless, and defined  by the following equation
\begin{equation}\label{eq: cfl number}
    c = \frac{u\Delta t}{\Delta x}
\end{equation}

The \acrfull{cfl} number is based on the work by~\cite{courantBerPartiellenDifferenzengleiehungen}.
...
The viscous diffusion term and its time derivative are resolved implicitly, whereas the convective term and the body force term are resolved explicitly \citep{janssonNekoModernPortable2024}.

\fxbmark{Stand}
\section{Generating rough surfaces}\label{sec:generating rough surfaces}
\fxinfo{nur marginale änderungen, muss nicht korrigiert werden}
...
The second surface, denoted by $Z_s$, manifests the power spectrum, but not the probability distribution.
